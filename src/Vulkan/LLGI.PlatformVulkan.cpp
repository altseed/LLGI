...
bool PlatformVulkan::ValidateLayers(std::vector<const char*> requiredLayers, const std::vector<VkLayerProperties>& properties)
{
	for (const auto& requiredLayer : requiredLayers)
	{
		...
		{
			if (strcmp(property.layerName, requiredLayer) == 0)
			{
				found = true;
				break;
			}
...
std::vector<const char*> PlatformVulkan::GetOptimalLayers(const std::vector<VkLayerProperties>& properties)
{
	...
	// not found.
	Log(LogType::Info, "Layers are not found.");
	return {};
}
